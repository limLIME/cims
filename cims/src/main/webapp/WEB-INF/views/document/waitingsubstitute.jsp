...
<table class="table table-striped table-advance table-hover">
	...
	<tbody>
		<c:forEach items="${requestScope.waitingSubstitute.list}" var="dvo">
			<tr>
				<td>${dvo.docNo}</td>
...
<br>
	<c:choose >
	<c:when test="${waitingSubstitute.pagingBean.previousPageGroup==true}">
	<a href="${initParam.root}doc_waitingSubstitute.do?page=${waitingSubstitute.pagingBean.startPageOfPageGroup-1}&empNo=${sessionScope.evo.empNo}">이전</a>
	</c:when>
	</c:choose>
	<c:forEach begin="${waitingSubstitute.pagingBean.startPageOfPageGroup}" 
	end="${waitingMySign.pagingBean.endPageOfPageGroup}" var="pagelist" >	
			<a href="${initParam.root}doc_waitingSubstitute.do?page=${pagelist}&empNo=${sessionScope.evo.empNo}">${pagelist} </a>
	</c:forEach>
	<c:choose>
		<c:when test="${waitingSubstitute.pagingBean.nextPageGroup==true}">
		<a href="${initParam.root}doc_waitingSubstitute.do?page=${waitingSubstitute.pagingBean.endPageOfPageGroup+1}&empNo=${sessionScope.evo.empNo}">다음</a>
		</c:when>
	</c:choose>
	<br>	<br>
	<div align="center">
