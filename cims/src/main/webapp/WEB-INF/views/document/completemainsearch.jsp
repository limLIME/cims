...
<div align="center">
	<input type="text"  id="completeMainSearchText" class="form-control" placeholder="Enter your search word" style="width: 400px">
	<button class="btn btn-primary" id="completeMainSearchBtn" value="Search" style="width: 50px; height: 32px">
	<i class="fa fa-search"></i></button>
</div>
<br>
	<c:choose >
	<c:when test="${completeMainSearchList.pagingBean.previousPageGroup==true}">
	<a href="${initParam.root}doc_completeMainSearch.do?page=${completeMainSearchList.pagingBean.startPageOfPageGroup-1}&empNo=${sessionScope.evo.empNo}">이전</a>
	</c:when>
	</c:choose>
	<c:forEach begin="${completeMainSearchList.pagingBean.startPageOfPageGroup}" end="${completeMainSearchList.pagingBean.endPageOfPageGroup}" var="pagelist" >	
			<a href="${initParam.root}doc_completeMainSearch.do?page=${pagelist}&empNo=${sessionScope.evo.empNo}">${pagelist} </a>
	</c:forEach>
	<c:choose>
		<c:when test="${completeMainSearchList.pagingBean.nextPageGroup==true}">
		<a href="${initParam.root}doc_completeMainSearch.do?page=${completeMainSearchList.pagingBean.endPageOfPageGroup+1}&empNo=${sessionScope.evo.empNo}">다음</a>
		</c:when>
	</c:choose>
	<br><br>	
	</div></div>
