...
<table class="table table-striped table-advance table-hover">
	...
	<tbody>
		<c:forEach items="${requestScope.completeSignSearchList.list}" var="dvo">
			<tr>
				<td>${dvo.docNo}</td>
...
<br>
	<c:choose >
	<c:when test="${completeSignSearchList.pagingBean.previousPageGroup==true}">
	<a href="${initParam.root}doc_completeSignSearch.do?page=${completeSignSearchList.pagingBean.startPageOfPageGroup-1}&empNo=${sessionScope.evo.empNo}">이전</a>
	</c:when>
	</c:choose>
	<c:forEach begin="${completeSignSearchList.pagingBean.startPageOfPageGroup}" end="${completeSignSearchList.pagingBean.endPageOfPageGroup}" var="pagelist" >	
			<a href="${initParam.root}doc_completeSignSearch.do?page=${pagelist}&empNo=${sessionScope.evo.empNo}">${pagelist} </a>
	</c:forEach>
	<c:choose>
		<c:when test="${completeSignSearchList.pagingBean.nextPageGroup==true}">
		<a href="${initParam.root}doc_completeSignSearch.do?page=${completeSignSearchList.pagingBean.endPageOfPageGroup+1}&empNo=${sessionScope.evo.empNo}">다음</a>
		</c:when>
	</c:choose>
	<br>	<br>
	<div align="center">
